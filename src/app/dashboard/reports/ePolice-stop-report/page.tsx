...
// import { useStopReport } from "@/hooks/reports/useStopReport";
import { useEPoliceSummary } from "@/hooks/reports/useEPoliceSummary";

const EPoliceStopReportPage: React.FC = () => {
  ...
  const { data, isFetching } = useEPoliceSummary({
    uniqueId: apiFilters.uniqueId,
    selectedDate: apiFilters.from, // We use 'from' as the selected date based on previous logic
  ...
  const handleFilterSubmit = useCallback((filters: FilterValues) => {
    const fromDate = filters.from || filters.date;
    const toDate = filters.to || filters.date;

    if (!filters.deviceId || !fromDate) {
...
export default EPoliceStopReportPage;
